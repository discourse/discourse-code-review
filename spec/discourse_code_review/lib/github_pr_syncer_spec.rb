...
describe DiscourseCodeReview::GithubPRSyncer do
  ...
  describe "#sync_pull_request" do
    ...
    context "when there is a close event" do
      let!(:closed_event) do
        DiscourseCodeReview::PullRequestEvent.create(:closed)
      end
    ...
    context "when the events contain a merged event" do
      let!(:merged_event) do
        DiscourseCodeReview::PullRequestEvent.create(:merged)
      end
    ...
    context "when the events contain a review thread started event" do
      let!(:review_thread) do
        DiscourseCodeReview::CommentThread.new(
          github_id: "github review thread id"
        )
      end
      ...
      context "when the context is nil" do
        ...
        it "creates review thread started posts" do
          ...
          expect(last_post_of_last_topic.raw).to(
            include(review_thread_started_event.body)
          )
        end
      end
      ...
      context "when the context is present" do
        let!(:context) do
          DiscourseCodeReview::CommentContext.new(
      ...
      let!(:syncer) do
        pr_service =
          MockGithubPRService.new(
            ...
            pull_request_events: {
              pr => [
                [event_info, review_thread_started_event],
                [event_info2, review_comment],
              ]
            }
          )
    ...
    context "when the events contain a re-opened event" do
      ...
      let!(:reopened_event) do
        DiscourseCodeReview::PullRequestEvent.create(:reopened)
      end
