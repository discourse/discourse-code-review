  ...
  describe State::CommitApproval do
    ...
    describe 'creates notifications upon approval' do
      ...
      it "doesn't consolidate notifications if they were created more than 6 hours ago" do
        ...
        second_pr = DiscourseCodeReview::PullRequest.new(owner: "owner", name: "name", issue_number: 102)

        State::CommitApproval.approve(topic, [approver, merged_by], pr: pr, merged_by: merged_by)
        first_notification = Notification.find_by(user: topic.user, notification_type: Notification.types[:code_review_commit_approved])
        first_notification.update!(created_at: 7.hours.ago)

        State::CommitApproval.approve(second_topic, [approver, merged_by], pr: second_pr, merged_by: merged_by)

        notifications = Notification.where(user: topic.user, notification_type: Notification.types[:code_review_commit_approved])
